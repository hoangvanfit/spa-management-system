...
export const validateMeeting = async ({ roomId, token }) => {
    ...
    if (!roomId || !token) {
        return { meetingId: null, err: "Thiếu thông tin roomId hoặc token" };
    }
    ...
    try {
        ...
        if (!response.ok) {
            return {
                meetingId: null,
                err: `Lỗi ${response.status}: ${response.statusText}`,
            };
        }
        ...
        if (data.roomId) {
            ...
            return { meetingId: null, err: data.error || "Không tìm thấy roomId" };
        }
    } catch (error) {
        console.error("Error validating meeting:", error);
        return {
            meetingId: null,
            err: `Lỗi kết nối: ${error.message}`,
        };
    }
};
