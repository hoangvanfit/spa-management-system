...
class StatisticalController extends Controller
{
    ...
    public function dailyRevenues(Request $request)
    {
        ...
        // Giới hạn ngày không được vượt quá ngày 14 của tháng hiện tại
        $maxDate = Carbon::now();
        if ($maxDate->day > 14) {
            $maxDate = Carbon::create($maxDate->year, $maxDate->month, 14);
        }
        
        // Nếu ngày được chọn vượt quá ngày 14, sử dụng ngày 14
        $selectedDate = Carbon::parse($day);
        if ($selectedDate->gt($maxDate)) {
            $day = $maxDate->format('Y-m-d');
        }

        // Tính toán doanh thu và số lượng (chỉ tính đến ngày được chọn)
    ...
    public function weeklyRevenues(Request $request)
    {
        ...
        // Giới hạn không được vượt quá ngày 14 của tháng hiện tại
        $maxDate = Carbon::now();
        if ($maxDate->day > 14) {
            $maxDate = Carbon::create($maxDate->year, $maxDate->month, 14)->endOfDay();
        }
        
        // Nếu endOfWeek vượt quá maxDate, sử dụng maxDate
        $endOfWeekDate = Carbon::parse($endOfWeek);
        if ($endOfWeekDate->gt($maxDate)) {
            $endOfWeek = $maxDate->format('Y-m-d H:i:s');
        }

        // Tính toán doanh thu và số lượng (chỉ tính đến ngày 14)
        // Sử dụng endOfWeek đã được giới hạn ở trên
    ...
    public function monthlyRevenues(Request $request)
    {
        ...
        // Nếu đang tính tháng hiện tại và endOfMonth vượt quá maxDate, sử dụng maxDate
        $currentYear = Carbon::now()->year;
        $currentMonth = Carbon::now()->month;
        if ($year == $currentYear && $month == $currentMonth) {
            $endOfMonthDate = Carbon::parse($endOfMonth);
            if ($endOfMonthDate->gt($maxDate)) {
                $endOfMonth = $maxDate->format('Y-m-d H:i:s');
            }
        }

        // Tính toán doanh thu và số lượng (chỉ tính đến ngày 14 nếu là tháng hiện tại)
        $query = Payment::whereBetween('created_at', [$startOfMonth, $endOfMonth]);
        if ($year == $currentYear && $month == $currentMonth) {
            $query->whereDate('created_at', '<=', $maxDate->format('Y-m-d'));
        }
        
        $total_revenue = (clone $query)->sum('total_amount');
        $completed_revenue = (clone $query)->where('status', 1)->sum('total_amount');
        $pending_revenue = (clone $query)->where('status', 0)->sum('total_amount');
        $cash_revenue = (clone $query)->where('payment_type', 0)->sum('total_amount');
        $transfer_revenue = (clone $query)->where('payment_type', 1)->sum('total_amount');

        // Đếm số lượng giao dịch
        $total_count = (clone $query)->count();
        $completed_count = (clone $query)->where('status', 1)->count();
        $pending_count = (clone $query)->where('status', 0)->count();
        $cash_count = (clone $query)->where('payment_type', 0)->count();
        $transfer_count = (clone $query)->where('payment_type', 1)->count();
