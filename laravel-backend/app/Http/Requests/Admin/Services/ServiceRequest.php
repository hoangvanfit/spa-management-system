...
class ServiceRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'image_url.*' => 'nullable|image', // Cho phép null nếu dùng existing_images
            'existing_images.*' => 'nullable|string', // Cho phép dùng ảnh có sẵn
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            // Đảm bảo có ít nhất một trong hai: image_url hoặc existing_images
            $hasImageUpload = $this->hasFile('image_url') && count($this->file('image_url')) > 0;
            $hasExistingImages = $this->has('existing_images') && is_array($this->input('existing_images')) && count($this->input('existing_images')) > 0;
            
            if (!$hasImageUpload && !$hasExistingImages) {
                $validator->errors()->add('image_url', 'Vui lòng chọn ít nhất một ảnh (upload mới hoặc chọn ảnh có sẵn).');
            }
        });
    }
