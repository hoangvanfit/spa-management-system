...
class User extends Authenticatable implements JWTSubject, MustVerifyEmail
{
    ...
    {
        // Tính tổng tiền từ các appointments mà staff tham gia
        return DB::table('appointment_staffs')
            ->join('appointments', 'appointment_staffs.appointment_id', '=', 'appointments.id')
            ->join('payments', 'appointments.id', '=', 'payments.appointment_id')
            ->where('appointment_staffs.staff_id', $this->id)
            ->whereDate('payments.created_at', $day)
            ->where('payments.status', 1) // Chỉ tính payments đã hoàn thành
            ->sum('payments.total_amount');
    }
    
    public function payment_type_cash($day)
    {
        // Tính tổng tiền mặt từ các appointments mà staff tham gia
        return DB::table('appointment_staffs')
            ->join('appointments', 'appointment_staffs.appointment_id', '=', 'appointments.id')
            ->join('payments', 'appointments.id', '=', 'payments.appointment_id')
            ->where('appointment_staffs.staff_id', $this->id)
            ->whereDate('payments.created_at', $day)
            ->where('payments.status', 1) // Chỉ tính payments đã hoàn thành
            ->where('payments.payment_type', 0) // Tiền mặt
            ->sum('payments.total_amount');
    }
    
    public function payment_type_transfer($day)
    {
        // Tính tổng chuyển khoản từ các appointments mà staff tham gia
        return DB::table('appointment_staffs')
            ->join('appointments', 'appointment_staffs.appointment_id', '=', 'appointments.id')
            ->join('payments', 'appointments.id', '=', 'payments.appointment_id')
            ->where('appointment_staffs.staff_id', $this->id)
            ->whereDate('payments.created_at', $day)
            ->where('payments.status', 1) // Chỉ tính payments đã hoàn thành
            ->where('payments.payment_type', 1) // Chuyển khoản
            ->sum('payments.total_amount');
    }
}
